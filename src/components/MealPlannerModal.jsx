...
export default function MealPlannerModal({ isOpen, onClose, onSubmitRecipe }) {
  ...
  const savePlanRecipes = (plan) => {
    if (plan && Array.isArray(plan.days)) {
      plan.days.forEach((day) => {
        if (Array.isArray(day.meals)) {
          day.meals.forEach((m) => {
            if (m && m.recipe) {
              historyManager.addGeneratedRecipe(m.recipe);
            }
          });
        }
      });
  ...
  const handleSwapMeal = async (dayIdx, mealIdx, type, currentName) => {
    if (!mealPlanResult || isGenerating) return;
    ...
    try {
      ...
      if (newRecipe) {
        historyManager.addGeneratedRecipe(newRecipe);
      }
      ...
      showToast(`✨ New ${type} recipe generated by ChefAI!`);

    } catch (error) {
      console.error("Swap meal error:", error);
      showToast("Unable to generate a new meal. Please try again.");
    } finally {
      setIsGenerating(false);
    }
  };
              ...
              <div className="grid grid-cols-3 gap-2">
                {['3', '5', '7'].map((d) => (
                  <button
                    ...
                    className={`py-2 px-3 rounded-xl border text-xs font-bold transition flex items-center justify-center min-h-[38px] ${preferences.duration === d
